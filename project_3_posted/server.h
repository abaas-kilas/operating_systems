...
void LogPrettyPrint(FILE* to_write, int threadId, int requestNumber, int file_descriptor, char* request_str, int num_bytes_or_error, bool cache_hit){
    if(to_write == NULL){
        printf("[Thread ID: %3d] [Request Number: %3d] [File Descriptor: %3d] [Request URL: %-30s] [Size (bytes): %7d] [Cache Hit? %5s]\n", threadId, requestNumber, file_descriptor, request_str, num_bytes_or_error, cache_hit? "TRUE" : "FALSE");
        fflush(stdout);
    }else{
        fprintf(to_write, "[Thread ID: %3d] [Request Number: %3d] [File Descriptor: %3d] [Request URL: %-30s] [Size (bytes): %7d] [Cache Hit? %5s]\n", threadId, requestNumber, file_descriptor, request_str, num_bytes_or_error, cache_hit? "TRUE" : "FALSE");
        fflush(to_write);
    }    
}
#endif /* _SERVER_H */
